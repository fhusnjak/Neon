...
layout(set = 1, binding = 1) uniform sampler2D blendMap;
layout(set = 1, binding = 2) uniform sampler2D backgroundTexture;
layout(set = 1, binding = 2) uniform sampler2D rTexture;
layout(set = 1, binding = 3) uniform sampler2D gTexture;
layout(set = 1, binding = 4) uniform sampler2D bTexture;
...
void main()
{
    ...
    vec3 finalTextureColor = background * texture(backgroundTexture, fragTileTexCoord) + blendColor.r * texture(rTexture, fragTileTexCoord) +
    ...
    vec3 diffuse = computeDiffuse(material, lightDir, fragNorm) * finalTextureColor;
